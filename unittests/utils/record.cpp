...
TEST_CASE("record requires input and output", "[record]") {
  ...
  SECTION("has input and output") {
    std::array<const char *, 5> testArgs = {"prog", "record", "-o", "test",
                                            "input"};
    const auto run = [&]() {
  ...
  SECTION("has two inputs") {
    std::array<const char *, 6> testArgs = {"prog", "record", "-o",
                                            "out",  "input1", "input2"};
    const auto run = [&]() {
      options opts{testArgs.size(), const_cast<char **>(testArgs.data()),
                   const_cast<char **>(env.data())};
  ...
  SECTION("has two outputs") {
    std::array<const char *, 7> testArgs = {"prog", "record", "-o",   "output",
                                            "-o",   "test",   "input"};
    const auto run = [&]() {
      options opts{testArgs.size(), const_cast<char **>(testArgs.data()),
                   const_cast<char **>(env.data())};
...
TEST_CASE("optional arguments are handled correctly", "[record]") {
  ...
  SECTION("has --skip-mem-objects") {
    std::array<const char *, 6> testArgs = {
        "prog", "record", "-o", "test", "--skip-mem-objects", "input"};
    const auto run = [&]() {
      options opts{testArgs.size(), const_cast<char **>(testArgs.data()),
                   const_cast<char **>(env.data())};
      REQUIRE(opts.input().string() == "input");
  ...
  SECTION("has --skip-mem-objects twice") {
    std::array<const char *, 7> testArgs = {"prog",
                                            "record",
                                            "-o",
                                            "test",
                                            "--skip-mem-objects",
  ...
  SECTION("has extra args") {
    std::array<const char *, 8> testArgs = {
        "prog", "record", "-o", "test", "input", "--", "--foo", "--bar"};
    const auto run = [&]() {
      options opts{testArgs.size(), const_cast<char **>(testArgs.data()),
                   const_cast<char **>(env.data())};
      REQUIRE(opts.input().string() == "input");
